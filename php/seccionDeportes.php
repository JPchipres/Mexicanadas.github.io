...
$numPosts = "SELECT id, title, description, image, created_at FROM post WHERE category_id=3 AND status=1 ORDER BY id ASC limit 0,4 ";
$consulta= $conn->query($numPosts);
$idPost= $consulta->fetchAll(\PDO::FETCH_ASSOC);
/**Noticia principal */
$principalPost = "SELECT id, title, description, image, created_at FROM post WHERE category_id=3 AND status=1 ORDER BY id ASC limit 0,1 ";
...
<head>
	...
	<title>Mexicanadas: Deportes</title>
