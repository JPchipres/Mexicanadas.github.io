...
if(isset($_POST['submit'])){
    ...
    if(!isset($errorMsg)){
        $sql = $conn->prepare("INSERT INTO post (title, autorid, description, content, image, category_id, link1, link2, link3) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $sql->execute(array($title_post, $autor_post, $description_post, $content_post, $ruta, $category_post, $url1, $url2, $url3));
        if($sql){
